...
static func load_sdf_texture(path: String, texture_size: int = 64) -> ImageTexture3D:
	...
	for z in range(texture_size):
		var start = z * bytes_per_slice
		var end = (z + 1) * bytes_per_slice
		var slice_data = data.slice(start, end)
		var img = Image.create_from_data(texture_size, texture_size, false, Image.FORMAT_R8, slice_data)
		images.append(img)
